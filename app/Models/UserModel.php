...
class UserModel extends Model
{
    ...
    public function usuarios($email,$clave)
    {
      /*  $dbo = \Config\Database::connect('oracle');
        //$dbo ??= Database::connect($this->DBOracle);
        $this->dbo = $dbo;

       $query = $this->dbo->query("SELECT * FROM CODIGOSS WHERE ROWNUM <= '20'");
      
       if ($query->getResultArray())
       {
           $result = $query->getResultArray();
           
          /* if (password_verify($clave, $result[0]["password"]))
           {
               return $result;
           }
           return $result;
