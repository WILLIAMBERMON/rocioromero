...
<div class="container-fluid">
  <div class="card card-dark">
    <div class="card-header">
      <h1 class="card-title"><strong>Lista de propietarios</strong></h1>
    ...
    <div class="card-body">
            ...
            <table id="example1s" class="datatablew table table-bordered table-striped">
              ...
              <tbody>
              <?php foreach($data['get_propietarios'] as $index => $prop):?>
                <tr>
                  ...
                  <td><button type="button" onclick="editarpropietario('<?php echo $prop['documento'];?>','<?php echo $prop['nombres'];?>','<?php echo $prop['email'];?>','<?php echo $prop['direccion'];?>','<?php echo $prop['celular'];?>','<?php echo $prop['estado'];?>')"  class="btn btn-flat btn-primary"><i class="fa-sharp fa-regular fa-pencil"></i></button></td>
                </tr>
              <?php endforeach;?>
                      </tbody>
                <tfoot>
    ...
    <div class="card-footer">
          <button type="button" onclick="newpropietario()"  class="btn btn-flat btn-success float-right" name="crear" value="1"> Crear Propietario</button>
      </div>
    <!-- /.card-body -->
  </div>
</div> 

<div style="display: <?php echo (isset($data['get_propietarios']) && $data['get_propietarios'])?'none':'block' ?>" id="addpro" class="container-fluid">
  <div class="card card-dark">
    <div class="card-header">
      <h1 class="card-title"><strong>Propietarios</strong></h1>
      
    </div>
    <!-- /.card-header -->
    <?php echo form_open(base_url('propietarios'), ['class' => 'form-horizontal', 'id' => 'form_dependencia', 'role' => 'form'],[]);?>
    <div class="card-body">
                <input type="hidden" name="agregarpropietario" value="1" id="agregarpropietario">
            <p  style="text-align:justify" >
              ...
              <label for="descripcion"><strong>Estado del propietario</strong></label>
    ...
    <div class="card-footer">
      <?php if(isset($data['get_propietarios']) && $data['get_propietarios']):?>
          ...
          <?php endif; ?>
          <button type="submit" id="crearbb" class="btn btn-flat btn-success float-right" name="crear" value="1"> Crear Propietario</button>
      </div>
      <?php echo form_close();?>
    <!-- /.card-body -->
  </div>
